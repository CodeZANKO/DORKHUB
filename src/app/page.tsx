  ...
  useEffect(() => {
    ...
    async function fetchInitialData() {
      ...
      try {
        ...
        if (dorksRes.error) {
          console.error('Fetch Error Detail:', dorksRes.error.message);
          toast.error("Failed to establish secure link to Dork Repository");
        }
        
        if (categoriesRes.data) setCategories(categoriesRes.data);

        if (dorksRes.data) {
          const mappedDorks: Dork[] = (dorksRes.data as unknown[]).map((item) => {
            const d = item as {
              id: string,
              query: string,
        ...
        <section id="repository" className="py-16 px-6 md:px-12 max-w-7xl mx-auto w-full space-y-12">
          ...
          {loading ? (
            <div className="h-96 flex flex-col items-center justify-center space-y-6">
              ...
              <p className="text-primary font-jetbrains text-xs font-bold uppercase tracking-[0.4em] animate-pulse">Loading...</p>
            </div>
          ) : view === 'categories' ? (
